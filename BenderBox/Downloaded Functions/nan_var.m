...
if isequal(w,0) || isequal(w,1)
    ...
    x0 = x - repmat(nan_mean(x, dim), tile);
    y = nan_sum(abs(x0).^2, dim) ./ denom; % abs guarantees a real result
...
elseif ~(isvector(w) && all(w(~isnan(w)) >= 0))
    ...
    % Count up non-NaNs.
    n = nan_sum(~isnan(x).*w,dim);

    x0 = x - repmat(nan_sum(w.*x, dim) ./ n, tile);
    y = nan_sum(w .* abs(x0).^2, dim) ./ n; % abs guarantees a real result
end
