...
for i=1:length(rawData)
    ...
    trial(counter).chamber = rawData{i,10};
    
    stimulus = strtrim(rawData{i,8});
    
    if strcmp(backgroundStim,stimulus)
        trial(counter).baseline.raw = [trial(counter).baseline.raw rawData{i,14}];
    elseif strcmp(startleStim,stimulus)
        trial(counter).startle.raw = [trial(counter).startle.raw rawData{i,14}];
    end
    
    for j=1:length(ppiStim)
        if strcmp(ppiStim{j},stimulus)
            trial(counter).ppi(j).raw = [trial(counter).ppi(j).raw rawData{i,14}];
        end
    end
